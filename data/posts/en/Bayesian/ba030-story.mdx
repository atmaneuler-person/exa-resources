...
- Regime 1 ($ S*t = 1 $): Delay regime. A state where a crack has occurred inside the supply chain, a swamp Marcus occasionally falls into. Once a delay occurs, inbound is pushed by an average of 8 days ($ \mu*{d1} $).
...
- $ P(d\_{\text{new}} \mid S_t = 1) $: How much the currently observed logistics indicators resemble the characteristics of the delay regime (likelihood).

(Since $ S*t $ is 0 if it is normal and 1 if it is a disaster state, $ P(d*{\text{new}} \mid S_t = 1) $ is nothing more than expressing the probability that the next delivery delay days will occur when the supplier is in a disaster state (worldview) as a formula. There is no need to be buried in symbols)
...
- Probability of entering catastrophe regime ($ \pi $): 16.5%

- Minimum mitigation amount ($ \Delta^\* $) to achieve the target probability of 95%: +7 days
...
This time the mail came accurately. But the content was not good.
...
"Marcus, I confirmed the two-day delay. We will optimize logistics on our side to shorten it by a day. Instead, if you can advance packing and shipment by even a day on your side, please do so. Please prepare in advance so that shipment is possible immediately upon completion of production."
...
"Honestly, I felt at ease too. I didn't know that not having to hide would increase work efficiency like this. Let's proceed like this for the next PO too."
